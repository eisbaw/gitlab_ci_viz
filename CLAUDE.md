...
# 10. COMMIT ATOMICALLY: Task + Code together
git add src/refactored.py backlog/tasks/task-042*.md
git commit -m "Refactor using strategy pattern (task-042)

- Applied strategy pattern for better extensibility
- Updated all affected tests
- Improved code documentation

Task-042 completed with all acceptance criteria met."
```

### ⚠️ CRITICAL: Atomic Commits

**Task updates and code changes MUST be committed together in a single atomic commit.**

**Workflow**:
1. Update task status/notes via backlog CLI (stages changes to `backlog/tasks/`)
2. Stage your implementation code changes with `git add`
3. Commit everything together with a meaningful message

**Never commit**:
- Task updates without code changes
- Code changes without updating corresponding task status

**Why**: This ensures the git history shows both what was built (code) and what was completed (task) in a single commit.
